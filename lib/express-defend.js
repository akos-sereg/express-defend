...
module.exports = {
	...
	suspiciousUrlFragments: [ 
		...
		// Reflected XSS
		'<script', '\\x3cscript', '%3cscript', 'alert(', 'console.log', 'onclick', 'onerror', 'onmouseover', 'onload' ],
	...
	handleSuspiciousRequest: function(request, response, next, blacklistItem) {
		...
		if (thresholdReached && this.onMaxAttemptsReached != null) {
			...
			this.onMaxAttemptsReached(request.connection.remoteAddress, request.originalUrl);
		}
